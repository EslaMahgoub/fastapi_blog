"""FastAPI blog application: web pages, API routes, and exception handlers."""
...
def home(request: Request, db: Annotated[Session, Depends(get_db)]):
    """Render home/posts list page with all posts from the database."""
    result = db.execute(select(models.Post))
    posts = result.scalars().all()
    return templates.TemplateResponse(
        "home.html", {"request": request, "posts": posts, "title": "YallaChat"}
    )


@app.get("/posts/{post_id}", include_in_schema=False, name="post_page")
def post_page(request: Request, post_id: int, db: Annotated[Session, Depends(get_db)]):
    """Render single post page by id; 404 if not found."""
...
def user_posts_page(
    ...
    """Render page listing all posts by the given user; 404 if user not found."""
...
def get_post(post_id: int, db: Annotated[Session, Depends(get_db)]):
    """Return post by id; 404 if not found."""
    result = db.execute(select(models.Post).where(models.Post.id == post_id))
    post = result.scalars().first()
    if post:
        return post
...
def general_http_exception_handler(request: Request, exception: StarletteHTTPException):
    """Handle HTTP exceptions: JSON for /api/*, HTML error page otherwise."""
    message = (
        exception.detail
        if exception.detail
    ...
    return templates.TemplateResponse(
        "error.html",
        {"request": request, "status_code": exception.status_code, "message": message},
...
def validation_exception_handler(request: Request, exception: RequestValidationError):
    """Handle validation errors: JSON detail for /api/*, HTML error page otherwise."""
    if request.url.path.startswith("/api"):
        return JSONResponse(
